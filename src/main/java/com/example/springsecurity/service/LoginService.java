...
import static com.example.springsecurity.common.config.security.jwt.JwtTokenProvider.REFRESH_TOKEN_VALID_TIME;
...
public class LoginService {
    ...
    public TokenDto login(LoginDto loginDto){
        ...
        redisService.setDataWithExpiration(REFRESH_TOKEN.getKey() + authenticate.getName(), refreshToken, REFRESH_TOKEN_VALID_TIME);
    ...
    public TokenDto reIssue(ReIssueDto reIssueDto){
        log.info("service reIssue");
        String findRefreshToken = redisService.getDate(REFRESH_TOKEN.getKey() + reIssueDto.getName());
        if(findRefreshToken == null || !findRefreshToken.equals(reIssueDto.getRefreshToken())){
            log.info("refreshToken이 일치하지 않습니다.");
            throw new InvalidRefreshTokenException();
        }

        Member member = memberRepository.findByName(reIssueDto.getName()).orElseThrow(() -> new MemberNotFoundException());

        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        String accessToken = jwtTokenProvider.createAccessToken(authentication);
        String refreshToken = jwtTokenProvider.createRefreshToken();
        redisService.setDataWithExpiration(REFRESH_TOKEN.getKey() + member.getName(), refreshToken, REFRESH_TOKEN_VALID_TIME);
